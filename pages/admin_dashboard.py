...
# -----------------------------------------------------------------------------
@st.cache_data(ttl=60) # Cache for 60 seconds to avoid hitting Azure too hard
...
def run_admin_portal():
    ...
    with col2:
        st.metric("Unique Learners", df['user_id'].nunique())
    with col3:
        st.metric("Global Avg Score", f"{df['percentage'].mean():.1f}%")
    with col4:
    ...
    with col_chart:
        ...
        # Group by quiz title to see which courses are hardest
    ...
    with col_table:
        ...
        # Count how many modules each unique user has taken
        user_activity = df.groupby('user_id').size().reset_index(name='Modules Completed')
        
        # Privacy filter: Mask the OID to just the first 8 characters
    ...
    # Format the log for clean reading
    display_df = df.sort_values(by='date', ascending=False).head(15) # Last 15 records
    ...
    st.dataframe(
        display_df[['Date', 'Learner', 'title', 'Score', 'percentage']], 
        hide_index=True, 
